...
clean-deduplication protocol (`reports/p2f_tighten.md`). Do **not** use the
earlier D5/D9 entry points (`reproducibility/run_d5_realdata.ps1`, or the
`src.experiments.d5` / `src.experiments.d5_baseline_expansion` modules) to
regenerate submission results; those target earlier intermediate protocols that
predate the leakage audit and may produce different numbers.
...
The external data root is configurable via the `GRAPHCOLD_DATA_ROOT`
environment variable (the legacy `GRAPH_COLD_DATA_ROOT` variable is still
honored). Set it to relocate the CESNET/UNSW trees without editing code; the
paths listed above reflect the authors' machine and are what the recorded audit
hashes were produced against.
...
The submission manuscript is `paper/graph_cold_cas_final.tex`; build it with
two `pdflatex` passes from the `paper/` directory.
